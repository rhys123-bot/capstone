<?php
   ...
   // owner LOGGED
   $sel_owner_logged_query = "SELECT *, c.status as `owner_verifiy`, LEFT(a.firstname, 1) as `initial_fname`, LEFT(a.email, 1) as `initial_email` FROM `owner_info` a 
   JOIN `user_account` b
   ...
   WHERE b.account_id = '$owner_id' AND d.status = 'active';";
   ...
   // // ownerS 
   // $sel_owner_query = "SELECT *, LEFT(a.email, 1) as initial_email FROM `owner_info` a 
   // JOIN `user_account` b
   // ON a.owner_id = b.account_id
   // JOIN `owner_temporary_account` c 
   // ON a.owner_id = c.owner_id
   // JOIN `owner_status` d
   // ON a.owner_id = d.owner_id
   // WHERE b.account_id != '$owner_id' AND d.status = 'inactive' ORDER BY a.id DESC;";

   // $sel_owner_res = mysqli_query($conn, $sel_owner_query);
